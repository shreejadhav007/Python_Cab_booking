...
class user:
    ...
    def login(self):
        mysqldb1 = mysql.connector.connect(host="localhost", user="root", password="", database="cab")
        mycursor1 = mysqldb1.cursor()
        print('hii')
        try:
            mycursor1.execute("SELECT * FROM cabb where user = '" + self.username.get() + " '")
            records1 = mycursor1.fetchall()
            for i in records1:
                if i[2] == self.username.get():
                    self.logf.pack_forget()
                    self.head['text'] = "Welcome " + self.username.get()
                    self.head.configure(fg="black")
                    self.head.pack(fill=X)
                    print('yes')
                    # ms.showinfo("Login successfull","thank you for login")
                    application=travel(root)
                else:
                    ms.showerror("Sorry","Create account first")
                    # window.window()
    ...
    def new_user(self):
        mysqldb = mysql.connector.connect(host="localhost", user="root", password="", database="cab")
        mycursor = mysqldb.cursor()

        try:
            sql = "INSERT INTO  cabb (user,pass) VALUES (%s,%s)"
            val = (self.n_username.get(),self.n_password.get())
    ...
    def widgets(self):
        ...
        self.head.pack()
        self.logf = Frame(self.master,padx =10,pady = 10,bd='2',relief=SUNKEN)
...
class travel:
    ...
    def __init__(self, root):
        ...
        def Kilo():
            if var2.get() == 0:
                ...
                if varl1.get() == "M.G.Road":
                    switch = {'Pune Station':10, "Magarpatta": 8, "Swargate": 6, "M.G.Road": 0}
                    Km.set(switch[varl2.get()])
                elif varl1.get() == "Swargate":
                    switch = {'Pune Station':4, "Magarpatta": 8, "Swargate": 0, "M.G.Road": 6}
                    Km.set(switch[varl2.get()])
                elif varl1.get() == "Magarpatta":
                    switch = {'Pune Station':10, "M.G.Road": 8, "Swargate": 8, "Magarpatta": 0}
                    Km.set(switch[varl2.get()])
                elif varl1.get() == "Pune Station":
                    switch = {'M.G.Road':10, "Magarpatta": 8, "Swargate": 4, "Pune Station": 0}
                    Km.set(switch[varl2.get()])
        ...
        def Total_Paid():
            if ((var1.get() == 1 and var2.get() == 1 and var3.get() <= 1 or var4.get() <= 1) and carType.get() != 0 and journeyType.get() != 0 and (varl1.get() != "" and varl2.get() != "")):
                if journeyType.get() == 1:
                    ...
                    # (fare+(insurance*km)+luggage+cartype)

                    Tax = "Rs " + str(((Cost_of_fare) * 0.09))
                    ST = "Rs " + str( ((Cost_of_fare)))
                    TT = "Rs " + str((Cost_of_fare + ((Cost_of_fare) * 0.9)))
                elif journeyType.get() == 2:
                    Item2 = Km.get()
                    Cost_of_fare = (Item1 + (float(Item2) * Item5)  + Item3 + Item4)*1.4
                    ...
                    Item2 = Km.get()
                    Cost_of_fare = (Item1 + (float(Item2) * Item5) + Item3 + Item4) *1.5
   ...
   #================================================mainframe========================================================================
    ...
    #===============================================Cab Information==============================================================
        ...
        self.cboPickup['value']=('','M.G.Road','Swargate','Magarpatta','Pune Station')
        self.cboPickup.current(0)
        ...
        self.cboDrop['value']=('','M.G.Road','Swargate','Magarpatta','Pune Station')
        self.cboDrop.current(0)
